...
function updateVehicleClassification() {
  ...
  $("#vehicle-classification").innerHTML = `<b>${label}</b><span>${vehicle.description}</span>`;
}
...
function renderHolidayList() {
  ...
  container.innerHTML = dates.map((date) => (
    `<span class="holiday-chip">${date}<button type="button" data-remove-holiday="${date}" aria-label="${date} 삭제">×</button></span>`
  )).join("");
